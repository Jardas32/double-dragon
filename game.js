...
window.addEventListener("keydown", (e) => {
  ...
  // Attack Hand
  if (e.key === "t" && !keys.right && !keys.left && !keys.up && !keys.down)
    keys.rCrocc = true;
  //Attack LoweKick
  if (e.key === "f" && !keys.right && !keys.left && !keys.up && !keys.down)
    keys.lkick = true;
});
...
function update() {
  ...
  if (
    playerBilly.attackZone &&
    isColliding(playerBilly.attackZone, enemyRoper)
  ) {
  ...
  if (
    playerBilly.attackZone &&
    isColliding(playerBilly.attackZone, enemyLinda)
  ) {
  ...
  if (
    playerBilly.attackZone &&
    isColliding(playerBilly.attackZone, enemyWillams)
  ) {
  ...
  if (
    enemyWillams.punchZone &&
    isColliding(enemyWillams.punchZone, playerBilly)
  ) {
    playerBilly.takeDamage(5);
  }
  ...
  if (
    playerBilly.attackZone &&
    isColliding(playerBilly.attackZone, bossAbobo)
  ) {
  ...
  if (cameraX > mapImg.width - canvas.width)
    cameraX = mapImg.width - canvas.width;
  if (cameraY > mapImg.height - canvas.height)
    cameraY = mapImg.height - canvas.height;
